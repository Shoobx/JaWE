      ...
      try {
         ...
         if (startingLocale != null && startingLocale.length() > 0) {
            if (!startingLocale.equals("default")) {
               PFLocale pfl = PFLocale.createPFLocale(startingLocale);
               choosenLocale = pfl.getLocale();
            } else {
               choosenLocale = defaultLocale;
            }
         } else {
         ...
         properties.put(AdditionalResourceManager.ADDITIONAL_LANGUAGE_PROPERTY_FILE
                        + "1", JaWEConstants.JAWE_LANGUAGE_MISC_PROPERTYFILE_NAME);
