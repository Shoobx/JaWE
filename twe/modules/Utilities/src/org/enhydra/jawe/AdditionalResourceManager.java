...
public class AdditionalResourceManager {
   ...
   public AdditionalResourceManager(Properties props) {
      List apfs = ResourceManager.getResourceStrings(props, ADDITIONAL_LANGUAGE_PROPERTY_FILE);
      if (apfs != null) {
         for (int i = 0; i < apfs.size(); i++) {
            String pf = (String) apfs.get(i);
            try {
               ResourceBundle bundle = ResourceBundle.getBundle(pf, ResourceManager.getChoosenLocale());
               ...
               if (loc.getLanguage().length() > 0) {
                  try {
                     Utils.manageProperties(eps, getCurrentConfigFolder(), locStrOrig + "_" + loc.getLanguage() + ".properties");
                  } catch (Exception ex) {
                  }
                  if (loc.getCountry().length() > 0) {
                     try {
                        Utils.manageProperties(eps, getCurrentConfigFolder(), locStrOrig + "_" + loc.getLanguage() + "_" + loc.getCountry() + ".properties");
                     } catch (Exception ex) {
                     }
                     if (loc.getVariant().length() > 0) {
                        try {
                           Utils.manageProperties(eps,
                                                  ...
                                                  locStrOrig + "_" + loc.getLanguage() + "_" + loc.getCountry() + "_" + loc.getVariant() + ".properties");
                        } catch (Exception ex) {
                        }
                     }
                  }
