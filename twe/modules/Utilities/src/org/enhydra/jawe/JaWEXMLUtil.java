...
/**
 * Class which purpose is to provide static methods which are used by classes that
 * represents program apstraction of XML elements. These methods offers support for
 * reading or writting an XML document and for generating the tooltips for for the classes
 * that needs it.
 */
public class JaWEXMLUtil {
   ...
   private static javax.swing.filechooser.FileFilter[] lastChoosenFilter = new javax.swing.filechooser.FileFilter[6];
   static {
   ...
   public static void message(String message, int type) {
      JaWEController jc = JaWEManager.getInstance().getJaWEController();
      JOptionPane.showMessageDialog(jc.getJaWEFrame(), message, jc.getSettings().getLanguageDependentString("DialogTitle"), type);
   }
   ...
   public static String dialog(Component parent, String message, int mode, int filteringMode, String initialName) {
      ...
      while (true) {
         ...
         if (returnVal == JFileChooser.APPROVE_OPTION) {
            ...
            // SAVING:
            // - if extension isn't specified, tries to save the file with default
            // extension
            // - it will not save file with extension if the file with extension already
            // exists
            ...
            if (mode == 1) {
               ...
               if (new File(fileName).exists()) {
                  int r = JOptionPane.showConfirmDialog(jc.getJaWEFrame(),
                                                        ...
                                                        jc.getSettings().getLanguageDependentString("DialogTitle"),
               ...
               if (fileName == null) {
                  JaWEXMLUtil.message(jc.getSettings().getLanguageDependentString("WarningFileDoesNotExistPleaseSelectExistingFileToOpen"),
