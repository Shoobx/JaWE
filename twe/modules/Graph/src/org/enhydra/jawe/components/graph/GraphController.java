...
public class GraphController extends Observable implements Observer, JaWEComponent, ChoiceButtonListener, GraphSelectionListener  {
   ...
   public GraphController(JaWEComponentSettings settings) throws Exception {
      ...
      c = Class.forName(this.settings.getGraphControllerPanel()).getConstructor(new Class[] {GraphController.class});
      panel = (GraphControllerPanel)c.newInstance(new Object[] {this});
...
//      JaWEManager.getInstance().getLoggingManager().info("GraphController -> trying to start update for event "+arg+", update in progress="+updateInProgress);
      ...
      if (info.getSource()==this || (changedElement==null && action != XPDLElementChangeInfo.SELECTED)) return;      
         ...
         if (action==XPDLElementChangeInfo.COPY || action==XPDLElementChangeInfo.CUT) {
            ...
            if (selectedGraph!=null) {
               ...
               while (it.hasNext()) {
                  ...
                  if (!(toCopyOrCut instanceof Activity)) continue;
   ...
   public void selectionChanged(ChoiceButton cbutton,Object change) {
      if (updateInProgress) return;
      if (getSelectedGraph()==null) return;
...
//         List vo=GraphUtilities.getParticipantVisualOrder(selectedGraph.getXPDLObject());
//         vo.add(par.getId());
...
//         getSelectedGraph().getGraphManager().insertParticipantAndArrangeParticipants(par);
   ...
   public List getChoices(ChoiceButton cbutton) {
      ...
      if (selectedGraph==null) return choices;
   ...
   public void valueChanged(GraphSelectionEvent e) {
      ...
      if (selectedElements.size() != 0) {
         if (par!=null) {
//            if (par instanceof FreeTextExpressionParticipant || par instanceof CommonExpressionParticipant) {
//               setUpdateInProgress(true);
         ...
         if (changedElement != null) {
            ...
            if (g == null && selectedGraph != null) {
               if (lastSel instanceof Participant
                     && selectedGraph.getGraphManager().getGraphParticipant((Participant) lastSel) != null) {
                  g = selectedGraph;
               } else if (lastSel instanceof ExtendedAttribute
                     && selectedGraph.getGraphManager().getBubble((ExtendedAttribute) lastSel) != null) {
                  g = selectedGraph;
               } else if (lastSel instanceof Activity
                     && selectedGraph.getGraphManager().getGraphActivity((Activity) lastSel) != null) {
                  g = selectedGraph;
               } else if (lastSel instanceof Transition
                     && selectedGraph.getGraphManager().getGraphTransition((Transition) lastSel) != null) {
                  g = selectedGraph;
               }
            }            
            ...
            if (selectedGraph != null) {
               ...
               for (Iterator it = toCheck.iterator(); it.hasNext();) {
                  ...
                  if (overview != null && selectedElement != null
...
//                  System.err.println("ADJUSTING WPORAS "+wpOrAs.getId()+" AFTER EPCHANGE");
            ...
            if (action == XMLElementChangeInfo.INSERTED || action == XMLElementChangeInfo.REMOVED
   ...
   public void createGraph (WorkflowProcess wp) {
      if (graphMap.get(wp)!=null) return;
...
//      panel.graphAdded(g);
   ...
   public void createGraph (ActivitySet as) {
      if (graphMap.get(as)!=null) return;
...
//      panel.graphSelected(selectedGraph);
   ...
   public void selectGraphForElement (XMLCollectionElement cel) {
      ...
      panel.graphSelected(selectedGraph);
      if (selectedGraph != null) {
         JaWEManager.getInstance().getJaWEController().getSelectionManager()
