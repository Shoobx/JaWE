...
public class GraphControllerPanel extends JPanel implements JaWEComponentView {
   ...
   public void init() {
      ...
      asChoiceButton = new XMLElementChoiceButton(ActivitySet.class, controller, curIc, true, new String[] {
         "Id"
      });
      ...
      JaWEAction ja = controller.getSettings().getAction("ActualSize");
      if (ja != null) {
         ...
         getActionMap().put("ActualSize", ja.getAction());
      }

      ja = controller.getSettings().getAction("ZoomIn");
      if (ja != null) {
         ...
         getActionMap().put("ZoomIn", ja.getAction());
      }

      ja = controller.getSettings().getAction("ZoomOut");
      if (ja != null) {
         ...
         getActionMap().put("ZoomOut", ja.getAction());
      }
   ...
   protected void addSetActivityModeComponent(JaWETypes jts, JToolBar toolbar) {
      ...
      for (int i = 0; i < actTypes.size(); i++) {
         ...
         b.setToolTipText(ja.getLangDepName());
         if (controller.getGraphSettings().hasAdditionalToolbar() && hasAdditionalActivities(jt)) {// ovo
                                                                                                   // je
                                                                                                   // sad
                                                                                                   // dodato
   ...
   protected boolean hasAdditionalActivities(JaWEType jt) {
      if (jt.getTypeId().equals(JaWEConstants.ACTIVITY_TYPE_NO)
          || jt.getTypeId().equals(JaWEConstants.ACTIVITY_TYPE_TASK_APPLICATION) || jt.getTypeId().equals(JaWEConstants.ACTIVITY_TYPE_TASK_SCRIPT)
          || jt.getTypeId().equals(JaWEConstants.ACTIVITY_TYPE_BLOCK) || jt.getTypeId().equals(JaWEConstants.ACTIVITY_TYPE_ROUTE_EXCLUSIVE)
          || jt.getTypeId().equals(JaWEConstants.ACTIVITY_TYPE_ROUTE_PARALLEL) || jt.getTypeId().equals(JaWEConstants.ACTIVITY_TYPE_SUBFLOW)) {
         return false;
      }
      return true;
   }
         ...
         if (XMLUtil.getPackage(controller.getSelectedGraph().getXPDLObject()) == JaWEManager.getInstance().getJaWEController().getMainPackage())
            if (controller.getChoices(showParticipantChoiceButton).size() != 0) {
               ...
               if (scells != null && scells.length > 0 && ((WorkflowElement) scells[0]).getPropertyObject() instanceof Lane) {
                  ...
                  GraphSwimlaneInterface gpar = controller.getSelectedGraph().getGraphManager().getGraphParticipant(parentL);
