...
//			return graphImage;
		...
		if (cells.length > 0) {
			...
			//graph.setScale(1.5);
			java.awt.Rectangle bounds = graph.getCellBounds(cells).getBounds();// HM, JGraph3.4.1
	...
	private void insertGraphImage(Document document, WorkflowProcess process) {
		...
		try{
			...
			if (graphImage !=null){
				...
				if ( ((pageRatio>1.0f) && (ratio<1.0f)) || ((pageRatio<1.0f) && (ratio>1.0f)) ){
				...
				graphImage.scaleToFit(graphImage.getScaledWidth()*scale,graphImage.getScaledHeight()*scale);
				...
				description.add(new Phrase(process.getProcessHeader().getDescription(),NORMAL_FONT));
	...
	private boolean addParameterRow(PdfPTable table, XMLElement elt, PDFChain chain) throws DocumentException{
		if (log.isLoggable(Level.FINEST))
			log.finest("creating row for element "+elt+"(name="+elt.toName()+")");
		Element visual = chain.startProcessElement(elt);
		if (visual !=null){
			table.addCell(makeBorderLessCell(new Phrase(JaWEManager.getInstance().getLabelGenerator().getLabel(elt),PARAMETER_FONT)));
	...
	private boolean addParameterRowNoId(PdfPTable table, XMLElement elt, PDFChain chain) throws DocumentException{
		if (log.isLoggable(Level.FINEST))
			log.finest("creating row for element "+elt+"(name="+elt.toName()+")");
		Element visual = chain.startProcessElement(elt);
		if (visual !=null){
	...
	private boolean addAttributeRow(PdfPTable table, ExtendedAttribute elt, PDFChain chain) throws DocumentException{
		Element visual = chain.startProcessElement(elt);
		if (visual !=null){
	...
	private PdfPTable makeDataFieldReport(DataField field, PDFChain chain) throws DocumentException{
	...
	private Image getTransitionPicture(JaWEType type, boolean incoming, String condition) throws Exception{
	...
	private Image getTransitionPicture(Transition t, boolean incoming) throws Exception{
		JaWEType type = JaWEManager.getInstance().getJaWEController().getTypeResolver().getJaWEType(t);
	...
	private List getActivityOutgoingTransitions(Activity activity){
		...
		for (Iterator i=childs.iterator();i.hasNext();){
			Object o = i.next();
			if ( (!(o instanceof Transition)) || (!((Transition)o).getFrom().equals(activity.getId())))
				i.remove();
		}
		return childs;
	}
	private List getActivityIngoingTransitions(Activity activity){
		...
		for (Iterator i=childs.iterator();i.hasNext();){
			Object o = i.next();
			if ( (!(o instanceof Transition)) || (!((Transition)o).getTo().equals(activity.getId())))
				i.remove();
		}
		return childs;
	}

	private PdfPTable makeActivityTable(Activity activity, PDFSectionChain chain) throws DocumentException {
		PdfPTable table = new PdfPTable(2);
		table.setWidthPercentage(100);
		table.setKeepTogether(true);
		Rectangle border = new Rectangle(0.0f,0.0f);
		border.setBorderWidth(0.0f);
		PdfPTable detailsTable = new PdfPTable(new float[]{0.3f,0.7f});
		addParameterRow(detailsTable,activity.get("Id"),chain);
		addParameterRow(detailsTable,activity.get("Name"),chain);
		addParameterRow(detailsTable,activity.get("Performer"),chain);
		addParameterRow(detailsTable,activity.get("Deadlines"),chain);	
		PdfPCell cell = makeBorderLessCell(detailsTable);
		table.addCell(cell);
		List transitions;
		detailsTable = new PdfPTable(new float[]{0.2f,0.3f,0.5f});
		...
		if (!transitions.isEmpty()){
			...
			for (Iterator i = transitions.iterator();i.hasNext();){
				...
				try{
					...
					detailsTable.addCell(makeBorderLessCell(new Phrase(t.getFrom(),TRANSITION_CONDITION_FONT)));
					detailsTable.addCell(makeBorderLessCell(new Phrase(condition,TRANSITION_CONDITION_FONT)));
				} catch (Exception e){
					e.printStackTrace();
				}
			}
		...
		if (!transitions.isEmpty()){
			...
			for (Iterator i = transitions.iterator();i.hasNext();){
				...
				try{
					...
					detailsTable.addCell(makeBorderLessCell(new Phrase(t.getTo(),TRANSITION_CONDITION_FONT)));
					detailsTable.addCell(makeBorderLessCell(new Phrase(condition,TRANSITION_CONDITION_FONT)));
				} catch (Exception e){
					e.printStackTrace();
				}
		...
		detailsTable = new PdfPTable(new float[]{0.3f,0.7f});
	...
	public Element processElement(PDFElementChain chain) throws DocumentException{
		if (log.isLoggable(Level.FINER)){
			if (chain.getElement()==null)
				...
				log.finer("processing element of type "+chain.getElement().getClass().getName()+ "(value="+chain.getElement().toValue()+")");
		}
		...
		String s = chain.getElement().toValue();
	...
	public Section processSection(PDFSectionChain chain) throws DocumentException{
		...
		log.warning("Could not handle section for "+chain.getElement()+" in code pdf plugin");
		return null;
	}
