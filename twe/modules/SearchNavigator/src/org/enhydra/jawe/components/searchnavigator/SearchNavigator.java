...
public class SearchNavigator implements Observer, TreeSelectionListener, JaWEComponent {
   ...
   public void update(Observable o, Object arg) {
      ...
      if (!(action == XPDLElementChangeInfo.REFERENCES
            || action == XPDLElementChangeInfo.SEARCH_RESULT || action == XPDLElementChangeInfo.VALIDATION_ERRORS || action == XMLElementChangeInfo.REMOVED
            || action == XPDLElementChangeInfo.UNDOABLE_ACTION_ENDED || action == XPDLElementChangeInfo.UNDO || action == XPDLElementChangeInfo.REDO))
         return;
   ...
   public void update(XPDLElementChangeInfo info) {
      ...
      try {
         ...
         if (action == XPDLElementChangeInfo.REFERENCES || action == XPDLElementChangeInfo.SEARCH_RESULT) {
            ...
            panel.refreshDisplay(info.getChangedElement(), info.getChangedSubElements());
         } else {
            if (action == XPDLElementChangeInfo.UNDOABLE_ACTION_ENDED || action == XPDLElementChangeInfo.UNDO || action == XPDLElementChangeInfo.REDO) {
