...
public class ExternalTypeDeclarations extends ActionBase {
   public ExternalTypeDeclarations(JaWEComponent jawecomponent) {
      super(jawecomponent);
   }
   ...
   public void actionPerformed(ActionEvent e) {
      ...
      while (it.hasNext()) {
         ...
         if (pkg != p) {
            toShow.addAll(pkg.getTypeDeclarations().toElements());
         }
      }
      ...
      String title = JaWEManager.getInstance()
         .getLabelGenerator()
         .getLabel(p.getExternalPackages())
                     + " - "
                     + JaWEManager.getInstance()
                        .getLabelGenerator()
                        .getLabel(p.getTypeDeclarations());
      JaWEManager.getInstance()
         .getTableEditor()
         .showTable(title, p.getTypeDeclarations(), toShow, columnsToShow);
   }
}
