...
/**
 * Creates a combo panel and a panel which type is determined by the choosen element from
 * combo panel.
 * 
 * @author Sasa Bojanic
...
public class XMLDataTypesPanel extends XMLBasicPanel {
   ...
   public XMLDataTypesPanel(PanelContainer pc,
                            DataTypes myOwner,
                            List choices,
                            String title,
                            boolean isEnabled,
      ...
      super(pc, myOwner, title, true, false, true, tooltip);

      PanelSettings ps = (PanelSettings) pc.getSettings();
      boolean hasBorder = ps.doesXMLDataTypesPanelHasBorder();

      Dimension dim = new Dimension(ps.getXMLDataTypesPanelWidth(),
                                    ps.getXMLDataTypesPanelHeight());

      final JScrollPane jsp = new JScrollPane();
      jsp.setAlignmentX(Component.LEFT_ALIGNMENT);
      jsp.setAlignmentY(Component.TOP_ALIGNMENT);

      pCombo = new XMLComboPanel(pc,
                                 myOwner,
                                 choices,
                                 false,
                                 false,
      ...
      pCombo.getComboBox().setSelectedItem(new XMLElementView(pc,
                                                              myOwner.getChoosen(),
