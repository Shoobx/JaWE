...
<!-- ********************************************************************
     $Id: verbatim.xsl,v 1.2 2006/09/21 11:50:57 sasa Exp $
     ********************************************************************
...
<xsl:template match="programlisting|screen|synopsis">
  ...
  <xsl:when test="$shade.verbatim != 0">
     <xsl:if test = "$enhydra.programlisting.enable = 'true'">
		      <fo:block id="{$id}"
		                white-space-collapse='false'
		                white-space-treatment='preserve'
		                linefeed-treatment='preserve'
		                xsl:use-attribute-sets="monospace.verbatim.properties shade.verbatim.style enhydra.programlisting.properties">
		        <xsl:choose>
    ...
    <xsl:otherwise>
    	 <xsl:if test = "$enhydra.programlisting.enable = 'true'">
		      <fo:block id="{$id}"
		                ...
		                white-space-treatment='preserve'
		                linefeed-treatment="preserve"
		                xsl:use-attribute-sets="monospace.verbatim.properties enhydra.programlisting.properties">
		        <xsl:choose>
		          <xsl:when test="$hyphenate.verbatim != 0 and function-available('exsl:node-set')">
		            <xsl:apply-templates select="exsl:node-set($content)" mode="hyphenate.verbatim"/>
		          </xsl:when>
