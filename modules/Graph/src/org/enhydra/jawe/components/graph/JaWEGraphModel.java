...
public class JaWEGraphModel extends UndoableEditSupport implements Serializable, GraphModel {
	...
	public class GraphModelEdit extends CompoundEdit implements GraphModelEvent.GraphModelChange {
		...
		public GraphModelEdit(Object[] inserted, Object[] removed, Map attributes, ConnectionSet connectionSet,
				ParentMap parentMap, String name) {
			super();
		...
		public Object[] filterParents(Map childCount, int children) {
			...
			while (it.hasNext()) {
				...
				if (entry.getValue() instanceof Integer) {
					if (((Integer) entry.getValue()).intValue() == children) list.add(entry.getKey());
				}
			}
			return list.toArray();
		}
		...
		protected void handleEmptyGroups(Object[] groups) {
			if (groups != null && groups.length > 0) {
				if (remove == null) remove = new Object[] {};
		...
		public void execute() {
			...
			Set tmp = new HashSet();
			if (attributes != null) tmp.addAll(attributes.keySet());
			if (parentMap != null) tmp.addAll(parentMap.getChangedNodes());
			// Note: One must also include the previous parents!
			if (connectionSet != null) tmp.addAll(connectionSet.getChangedEdges());
			if (remove != null) {
			...
			changed = tmp.toArray();
			// Context cells
			List ctx = getEdges(JaWEGraphModel.this, changed);
			context = ctx.toArray();
			// Do Execute
			inserted = insert;
			removed = remove;
		...
		public void putViews(GraphLayoutCache view, CellView[] views) {
			if (view != null && views != null) cellViews.put(view, views);
		}
	...
	public class GraphModelLayerEdit extends AbstractUndoableEdit implements GraphModelEvent.GraphModelChange {
		...
		public GraphModelLayerEdit(Object[] cells, int layer) {
			...
			Set par = new HashSet();
			for (int i = 0; i < cells.length; i++) {
				Object cell = JaWEGraphModel.this.getParent(cells[i]);
				if (cell == null) cell = cells[i];
				par.add(cell);
			}
			changed = par.toArray();
		}
