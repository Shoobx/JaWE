...
public class XMLTabbedPanel extends XMLBasicPanel {
   ...
   public XMLTabbedPanel (
         PanelContainer pc,
         XMLElement myOwnerL,
         List panels,
         String title,
         boolean showTitle) {
      super(pc,myOwnerL,title,true,showTitle && title!=null && !title.equals(""),true);
      ...
      for (int i=0; i<panels.size(); i++) {
         ...
         if (pnl.getTitle()!=null && !pnl.getTitle().equals("")) {
            ...
            if (pnl.getOwner() instanceof XMLCollection) {
               t=pc.getSettings().getLanguageDependentString(pnl.getOwner().toName()+"Key");
            }
            tabbed.addTab(t,pnl);
         }
         else {
